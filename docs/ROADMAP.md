...
| T4.d | Corrections de la revue adverse : doublon panier repris puis envoyé, perte d'un panier montré, retrait qui renouvelle le code, blocage de la tablée, admission depuis le panier montré, envoi en suspens toujours tranché, avis réservé aux convives présents, droit `feedback.read` rattrapé sur les rôles existants *(D-108)* | fait (2026-09-25) |
...
- Écarts assumés à la fin de T4 : `hybrid` reporté *(D-094)* ; pas de réattribution d'une ligne ni
  ...
  *(D-103)* ; recommandations tirées des ventes reportées à T6 *(D-104)* ; les plats envoyés par
  ...
  quatre navigateurs, une réponse coupée et un double appui. Au déploiement :
  `npx convex run migrations:grantTemplatePermission '{"permission":"feedback.read"}'` donne la
  lecture des avis aux rôles déjà créés ; une table ouverte avant la mise à jour n'a pas de code
  (« Renouveler » en tire un).
