...
# En CI, `convex dev` déploie encore en tâche de fond pendant que ce script pose les variables :
# une écriture peut alors échouer en « OptimisticConcurrencyControlFailure » (503), une erreur
# que Convex dit transitoire. On réessaie, avec un délai croissant ; au-delà, l'échec est réel.
setenv() {
  for delay in 1 2 4 8 16; do
    pnpm exec convex env set "$1" "$2" && return 0
    echo "Nouvel essai de $1 dans ${delay} s" >&2
    sleep "$delay"
  done
  pnpm exec convex env set "$1" "$2"
}
setenv SITE_URL "http://localhost:3000"
has BETTER_AUTH_SECRET || setenv BETTER_AUTH_SECRET "$(random)"
has GUEST_PASS_SECRET || setenv GUEST_PASS_SECRET "$(random)"
has PIN_PEPPER || setenv PIN_PEPPER "$(random)"
...
setenv RESEND_API_KEY "test-local"
setenv EMAIL_FROM "Joliba <noreply@joliba.test>"
setenv RESEND_API_URL "http://127.0.0.1:4010/emails"
...
has PAYMENT_SECRETS_KEY || setenv PAYMENT_SECRETS_KEY "$(random)"
setenv JOLIBA_FAKE_PAYMENTS "1"
setenv WAVE_API_URL "http://127.0.0.1:4020"
# Autorise `scripts/seed-demo.mjs` (restaurant de démonstration, convex/devSeed.ts).
setenv JOLIBA_DEMO_SEED "1"
